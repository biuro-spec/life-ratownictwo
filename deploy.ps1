# Deploy life-ratownictwo.pl -> CyberFolks (WinSCP, FTP)
...
# --- poswiadczenia z FileZilli (recentservers.xml) ---
...
$REMOTE = '/domains/life-ratownictwo.pl/public_html'

$lines = @(
  'option batch abort',
  'option confirm off',
  ('open ftp://ntroixgelh@s75.cyber-folks.pl:21 -passive=on -password="' + $passQ + '"'),
  ('synchronize remote -criteria=size "' + "$PSScriptRoot\dist" + '" "' + $REMOTE + '"'),
  'exit'
)
$tmp = "$env:TEMP\ws_deploy.txt"
Set-Content $tmp $lines -Encoding ascii

Write-Host ">> Wysylka dist/ -> $REMOTE ..." -ForegroundColor Cyan
cmd /c "`"C:\Program Files (x86)\WinSCP\WinSCP.com`" /script=`"$tmp`" /ini=nul > `"$env:TEMP\ws_deploy_out.txt`" 2>&1"
$kod = $LASTEXITCODE
$out = Get-Content "$env:TEMP\ws_deploy_out.txt"
Remove-Item $tmp,"$env:TEMP\ws_deploy_out.txt" -Force
$out | Select-Object -Last 15
if ($kod -ne 0) { throw "WinSCP zakonczyl z kodem $kod" }
Write-Host ">> OK - https://life-ratownictwo.pl zaktualizowane." -ForegroundColor Green
